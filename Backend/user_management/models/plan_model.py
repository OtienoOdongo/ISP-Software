...
class ClientAnalytics(models.Model):
    client = models.ForeignKey('account.Client', on_delete=models.CASCADE, related_name='analytics')
    timestamp = models.DateTimeField(auto_now_add=True)
    data_usage_percentage = models.FloatField(null=True, blank=True)
    payment_status = models.CharField(max_length=20, choices=[
        ('Paid', 'Paid'),
    ...
    plan = models.ForeignKey('internet_plans.InternetPlan', on_delete=models.SET_NULL, null=True, blank=True)
    hotspot_user = models.ForeignKey('network_management.HotspotUser', on_delete=models.SET_NULL, null=True, blank=True)
    is_high_usage = models.BooleanField(default=False)
    is_near_expiry = models.BooleanField(default=False)
...
class SMSNotification(models.Model):
    ...
    client = models.ForeignKey('account.Client', on_delete=models.CASCADE, related_name='sms_notifications')
    message = models.TextField()
    status = models.CharField(max_length=10, choices=STATUS_CHOICES, default='PENDING')
    trigger = models.CharField(max_length=20, choices=TRIGGER_CHOICES)
    sent_at = models.DateTimeField(null=True, blank=True)

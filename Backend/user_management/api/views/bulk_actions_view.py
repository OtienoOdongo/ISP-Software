...
class BulkUserListView(APIView):
    ...
    def get(self, request):
        ...
        clients = Client.objects.all().select_related('user', 'subscription__internet_plan')
        
        if search:
            clients = clients.filter(
                models.Q(user__username__icontains=search) |
                models.Q(user__phone_number__icontains=search) |
                models.Q(subscription__internet_plan__name__icontains=search)
            )
        ...
        if plan_filter != 'all':
            clients = clients.filter(subscription__internet_plan__name=plan_filter)
...
class BulkActionView(APIView):
    ...
    def post(self, request):
        ...
        clients = Client.objects.filter(id__in=user_ids)
        success_count = 0
        errors = []
    ...
    def _send_bulk_message(self, clients, message, message_type, subject):
        ...
        for client in clients:
            try:
                ...
                # Send message
                response = sms.send(formatted_message, [client.user.phone_number], settings_obj.sender_id)
            ...
            try:
                if action_type == 'activate':
                    ...
                    if client.subscription:
                        client.subscription.is_active = True
    ...
    def _delete_users(self, clients):
        ...
        for client in clients:
            try:
                # Soft delete - deactivate user and subscription

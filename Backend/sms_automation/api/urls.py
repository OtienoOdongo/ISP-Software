...
# URL Patterns - FIXED: Remove the 'api/sms/' prefix since it's already in main urls.py
urlpatterns = [
    # Router URLs - these will be under /api/sms/
    path('', include(router.urls)),
    
    # Analytics & Dashboard
    path('analytics/', SMSAnalyticsView.as_view(), name='sms-analytics'),
    path('dashboard/', SMSDashboardView.as_view(), name='sms-dashboard'),
    
    # Processing endpoints
    path('process-scheduled/', ProcessScheduledMessagesView.as_view(), name='process-scheduled-messages'),
    path('process-retry/', ProcessRetryMessagesView.as_view(), name='process-retry-messages'),
    
    # Webhook endpoints
    path('webhook/<str:gateway_type>/', SMSWebhookView.as_view(), name='sms-webhook'),
    path('webhook/', SMSWebhookView.as_view(), name='sms-webhook-generic'),
    
    # Message status updates
    path('messages/<uuid:message_id>/status/', UpdateMessageStatusView.as_view(), name='update-message-status'),
    
    # Specialized sending
    path('send-pppoe-credentials/', SendPPPoECredentialsView.as_view(), name='send-pppoe-credentials'),
    
    # Health check
    path('health/', SMSDashboardView.as_view(), name='sms-health'),
]
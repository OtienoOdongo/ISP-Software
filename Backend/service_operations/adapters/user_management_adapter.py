...
Enhanced with circuit breaker patterns, signal emissions, and robust error handling
Fully integrated with Service Operations architecture
UPDATED: Added get_client_by_uuid() method and improved UUID/phone handling
"""
import logging
import requests
import uuid as uuid_module
...
class UserManagementAdapter:
    ...
    # ==================== NEW METHOD: Get client by UUID ====================
    
    @classmethod
    def get_client_by_uuid(cls, client_uuid: str) -> Optional[Dict[str, Any]]:
        """
        Get client details directly by UUID from authentication app
        Uses the correct endpoint from authentication app URLs
        """
        cache_key = f"client_uuid:{client_uuid}"
        
        # Return cached if available
        cached_details = cache.get(cache_key)
        if cached_details:
            logger.debug(f"Returning cached client details for UUID {client_uuid}")
            return cached_details
        
        # Check circuit breaker
        circuit_ok, circuit_error = cls._check_circuit_breaker('client_details')
        if not circuit_ok:
            logger.warning(f"Circuit blocked for client UUID lookup: {circuit_error}")
            return None
        
        try:
            # Use the authentication app endpoint that exists
            # From your authentication app URLs: /api/auth/clients/<uuid:client_id>/
            endpoint = f"{cls.BASE_URL}/api/auth/clients/{client_uuid}/"
            ...
            if response and response.status_code == 200:
                ...
                if client_data:
                    ...
                    logger.info(f"Retrieved client details for UUID {client_uuid}")
                    return enriched_data
            
            # If we get here, client not found
            logger.warning(f"Client not found for UUID: {client_uuid}")
            cache.set(cache_key, None, 60)  # Cache not found for 1 minute
            return None
            
        except Exception as e:
            logger.error(f"Error getting client by UUID {client_uuid}: {e}")
            
            OperationLog.log_operation(
                operation_type='client_uuid_lookup_error',
                severity='error',
                description=f"Failed to get client by UUID {client_uuid}",
                details={'client_uuid': client_uuid, 'error': str(e)},
                source_module='user_management_adapter',
                source_function='get_client_by_uuid'
            )
            
            return None
    
    # ==================== UPDATED METHOD: get_client_details ====================
    
    @classmethod
    def get_client_details(cls, client_identifier: str, include_sensitive: bool = False) -> Optional[Dict[str, Any]]:
        """
        Get comprehensive client details from User Management app with caching
        Enhanced to better handle UUID lookup
        """
        cache_key = f"client_details:{client_identifier}:{include_sensitive}"
        ...
        try:
            # Check if it's a UUID
            try:
                uuid_obj = uuid_module.UUID(str(client_identifier))
                # It's a valid UUID, use the dedicated UUID endpoint
                return cls.get_client_by_uuid(client_identifier)
                
            except (ValueError, TypeError):
                # Not a UUID, try phone number lookup
                pass
            
            # If we get here, it's likely a phone number
            normalized_phone = cls._normalize_phone(client_identifier)
            if not normalized_phone:
                logger.warning(f"Invalid phone number format: {client_identifier}")
                return None
            
            # Search by phone using the correct endpoint
            endpoint = f"{cls.BASE_URL}/api/auth/clients/search/?phone_number={normalized_phone}"
            response = cls._make_request_with_retry('GET', endpoint, service='client_details')
            
            if response and response.status_code == 200:
                data = response.json()
                
                # Handle the response structure from your auth app
                if data.get('success') and data.get('client') and len(data['client']) > 0:
                    client_data = data['client'][0]  # First client in list
                    ...
                    logger.info(f"Retrieved client details for phone {normalized_phone}")
                    return enriched_data

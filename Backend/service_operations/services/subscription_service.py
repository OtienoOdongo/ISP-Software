...
# Core business logic for subscription lifecycle management
# References only to other apps - NO user creation or payment processing
# """
...
#     References only - NO user creation or payment processing
#     """
...
#         Client must already exist in Authentication app
#         Plan must already exist in Internet Plans app
#         """
#         try:
#             with transaction.atomic():
#                 # Validate client type
#                 if client_type not in ['hotspot_client', 'pppoe_client']:
#                     raise ValueError(f"Invalid client type: {client_type}")
                
#                 # Validate client type specific data
#                 if client_type == 'hotspot_client' and not hotspot_mac_address:
#                     raise ValueError("Hotspot client requires MAC address")
#                 elif client_type == 'pppoe_client' and (not pppoe_username or not pppoe_password):
#                     raise ValueError("PPPoE client requires username and password")
                
#                 # Calculate end date
#                 start_date = timezone.now()
#                 end_date = start_date + timezone.timedelta(hours=duration_hours)
                
#                 # Get plan technical configuration via adapter
...
#                     logger.warning(f"Could not get plan technical config: {e}")
                
#                 # Create subscription with correct field names
#                 subscription_data = {
#                     'client_id': client_id,
...
#                 cls._calculate_limits_from_plan(subscription)
...
#                     description=f"Subscription created for client {client_id}",
#                     details={
#                         'client_id': client_id,
#                         'plan_id': internet_plan_id,
#                         'client_type': client_type,
...
#                 severity='error',
#                 description=f"Failed to create subscription: {e}",
#                 details={
#                     'client_id': client_id,
#                     'plan_id': internet_plan_id,
#                     'client_type': client_type,
...
#     @classmethod
#     def _calculate_limits_from_plan(cls, subscription: Subscription):
#         """
#         Calculate data and time limits from plan via adapter
#         """
#         try:
#             # Get plan configuration via adapter
#             plan_config = InternetPlansAdapter.get_plan_technical_config(
#                 plan_id=str(subscription.internet_plan_id),
#                 access_method=subscription.access_method
#             )
            
#             # Extract limits
#             data_limit = plan_config.get('data_limit', {}).get('bytes', 0)
#             time_limit = plan_config.get('time_limit', {}).get('seconds', 0)
            
#             # Set limits
#             subscription.data_limit_bytes = data_limit
#             subscription.time_limit_seconds = time_limit
#             subscription.remaining_data_bytes = data_limit
#             subscription.remaining_time_seconds = time_limit
...
#         except Exception as e:
#             logger.error(f"Failed to calculate limits from plan: {e}")
#             # Set default limits
#             subscription.data_limit_bytes = 10 * 1024 * 1024 * 1024  # 10GB default
#             subscription.time_limit_seconds = 24 * 3600  # 24 hours default
#             subscription.remaining_data_bytes = subscription.data_limit_bytes
#             subscription.remaining_time_seconds = subscription.time_limit_seconds
#             subscription.save(update_fields=[
#                 'data_limit_bytes', 'time_limit_seconds',
#                 'remaining_data_bytes', 'remaining_time_seconds'
#             ])
...
#         payment_reference comes from Payment app
#         """
#         try:
#             with transaction.atomic():
...
#                 # Request activation via activation service
...
#                             description=f'Subscription activated via {payment_method}',
#                             status='completed',
...
#                         logger.info(f"Subscription activation requested: {subscription_id}")
#                         return True
#                     else:
#                         subscription.status = 'failed'
#                         subscription.activation_error = activation_result.get('error', 'Activation failed')
#                         subscription.save(update_fields=['status', 'activation_error', 'updated_at'])
                        
#                         logger.error(f"Subscription activation failed: {subscription_id} - {activation_result.get('error')}")
#                         return False
#                 else:
#                     # Schedule activation via queue
...
#                     logger.info(f"Subscription scheduled for activation: {subscription_id}")
#                     return True
...
#                 description=f"Failed to activate subscription: {e}",
#                 details={
#                     'subscription_id': subscription_id,
#                     'payment_reference': payment_reference,
#                     'error': str(e)
...
#     ) -> Dict[str, Any]:
#         """
#         Update subscription usage atomically
#         Called by Network Management app
#         """
#         try:
#             with transaction.atomic():
#                 subscription = Subscription.objects.select_for_update().get(
...
#                     subscription=subscription,
#                     session_id=session_id or f"session-{subscription_id}-{int(timezone.now().timestamp())}",
#                     data_used_bytes=data_used_bytes,
#                     session_duration_seconds=time_used_seconds,
...
#                     severity='info',
#                     subscription=subscription,
#                     description=f"Usage updated: {data_used_bytes} bytes, {time_used_seconds} seconds",
#                     details={
#                         'data_used_bytes': data_used_bytes,
...
#                 logger.info(f"Usage updated for subscription {subscription_id}: {data_used_bytes} bytes, {time_used_seconds} seconds")
...
#         except Exception as e:
#             logger.error(f"Failed to update usage for subscription {subscription_id}: {e}")
#             OperationLog.log_operation(
#                 operation_type='usage_update_failed',
#                 severity='error',
#                 subscription_id=subscription_id,
#                 description=f"Failed to update usage: {e}",
#                 details={
#                     'subscription_id': subscription_id,
#                     'error': str(e)
#                 },
...
#                     raise ValueError("Subscription cannot be renewed")
...
#                             access_method=subscription.access_method
#                         )
#                         duration_hours = plan_config.get('default_duration_hours', 24)
#                     except:
#                         duration_hours = 24
...
#                     new_end_date = subscription.end_date + timezone.timedelta(hours=duration_hours)
#                 else:
#                     # Start from now
#                     new_end_date = timezone.now() + timezone.timedelta(hours=duration_hours)
                
#                 # Update subscription
#                 subscription.end_date = new_end_date
#                 subscription.status = 'active'
                
#                 # Reset usage for renewal if expired
#                 if subscription.status == 'expired':
#                     cls._calculate_limits_from_plan(subscription)
#                     subscription.used_data_bytes = 0
#                     subscription.used_time_seconds = 0
#                     subscription.last_usage_update = None
                
#                 subscription.save()
                
#                 # Request re-activation via activation service
#                 activation_result = ActivationService.request_subscription_activation(subscription)
                
#                 if not activation_result.get('success'):
#                     logger.warning(f"Renewal activation failed for subscription {subscription_id}: {activation_result.get('error')}")
...
#                     severity='info',
#                     subscription=subscription,
#                     description=f"Subscription renewed for {duration_hours} hours",
#                     details={
...
#                 logger.info(f"Subscription renewed: {subscription_id} for {duration_hours} hours")
#                 return True
...
#                 severity='error',
#                 subscription_id=subscription_id,
#                 description=f"Failed to renew subscription: {e}",
#                 details={
#                     'subscription_id': subscription_id,
#                     'error': str(e)
#                 },
...
#                 # Request deactivation via network adapter
#                 try:
#                     NetworkAdapter.deactivate_subscription(str(subscription.id))
#                 except Exception as e:
#                     logger.warning(f"Failed to deactivate subscription on network: {e}")
...
#                 severity='error',
#                 subscription_id=subscription_id,
#                 description=f"Failed to cancel subscription: {e}",
#                 details={
#                     'subscription_id': subscription_id,
#                     'reason': reason,
#                     'error': str(e)
...
#     @classmethod
#     def process_expired_subscriptions(cls) -> int:
#         """
...
#                             NetworkAdapter.deactivate_subscription(str(subscription.id))
#                         except Exception as e:
#                             logger.warning(f"Failed to deactivate expired subscription on network: {e}")
...
#         except Exception as e:
#             logger.error(f"Failed to process expired subscriptions: {e}")
#             return 0
    
#     @classmethod
#     def auto_renew_subscriptions(cls) -> Dict[str, int]:
#         """
...
#                 try:
#                     success = cls.renew_subscription(str(subscription.id))
...
#                 except Exception as e:
#                     logger.error(f"Failed to auto-renew subscription {subscription.id}: {e}")
#                     failed_count += 1
...
#                 'failed': failed_count,
#                 'total': renewed_count + failed_count
#             }
            
#         except Exception as e:
#             logger.error(f"Failed to auto-renew subscriptions: {e}")

...
Production-ready subscription activation service with complete method implementations
Handles subscription activation via NetworkManagement API with retry logic, circuit breaker, and health checks
Updated: Added explicit signal emissions in process_activation_queue_item (e.g., subscription_activated on success).
"""
...
from service_operations.adapters.network_adapter import NetworkAdapter
from service_operations.adapters.internet_plans_adapter import InternetPlansAdapter
from service_operations.signals.operation_signals import subscription_activated, activation_completed, activation_failed
...
class ActivationService:
    ...
    def request_subscription_activation(cls, subscription: Subscription) -> Dict[str, Any]:
        ...
        # Check concurrent activation limit
        if not cls._can_start_activation():
            logger.warning(f"Maximum concurrent activations reached, activation queued for subscription {subscription.id}")
            return cls._queue_activation(subscription)
    ...
    def _prepare_activation_data(cls, subscription: Subscription) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"Could not get plan technical config for subscription {subscription.id}: {e}", exc_info=True)
            technical_config = {}
    ...
    def check_activation_status(cls, queue_item_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Status check error for queue item {queue_item_id}: {e}", exc_info=True)
            cls._record_failure()
    ...
    def process_activation_queue_item(cls, queue_item_id: str, processor_id: str = "system") -> Dict[str, Any]:
        ...
        Process a specific activation queue item.
        Called by queue service workers.
        Updated: Added explicit signal emissions on success/failure (subscription_activated, activation_completed/failed).
        """
        try:
            # Get queue item with subscription
            queue_item = ActivationQueue.objects.select_related('subscription').get(id=queue_item_id)
            ...
            if response.get('success'):
                ...
                activation_completed.send(
                    sender=ActivationQueue,
                    activation_id=str(queue_item.id),
                    subscription_id=str(subscription.id),
                    success=True,
                ...
                activation_failed.send(
                    ...
                    activation_id=str(queue_item.id),
                    subscription_id=str(subscription.id),
                    error_message=response.get('error', 'Activation failed'),
    ...
    def cancel_activation(cls, subscription_id: str, reason: str = "Cancelled by user") -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Failed to cancel activation on network for {subscription_id}: {e}", exc_info=True)
            ...
            logger.error(f"Failed to cancel activation for subscription {subscription_id}: {e}", exc_info=True)
            return {
                'success': False,
                'error': str(e),
                'timestamp': timezone.now().isoformat()
    ...
    def _handle_activation_error(cls, subscription: Subscription, response: Dict, duration: float) -> Dict[str, Any]:
        ...
        logger.error(f"Activation failed for subscription {subscription.id}: {error_msg}", exc_info=True)
    ...
    def _handle_timeout_error(cls, subscription: Subscription) -> Dict[str, Any]:
        """Handle timeout error."""
        logger.error(f"Activation timeout for subscription {subscription.id}", exc_info=True)
    ...
    def _handle_connection_error(cls, subscription: Subscription) -> Dict[str, Any]:
        """Handle connection error."""
        logger.error(f"Connection error for subscription {subscription.id}", exc_info=True)
    ...
    def _handle_unexpected_error(cls, subscription: Subscription, error: Exception) -> Dict[str, Any]:
        """Handle unexpected error."""
        error_msg = str(error)
        logger.error(f"Unexpected error for subscription {subscription.id}: {error_msg}", exc_info=True)
    ...
    def get_network_management_status(cls) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Failed to get network management status: {e}", exc_info=True)
            return {
                'success': False,
                'error': str(e),
                'timestamp': timezone.now().isoformat()
    ...
    def health_check(cls) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Activation service health check failed: {e}", exc_info=True)

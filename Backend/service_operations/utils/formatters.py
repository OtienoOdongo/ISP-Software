...
def calculate_usage_percentage(used: float, limit: float) -> float:
    """
    Calculate usage percentage safely with edge case handling
    
    Args:
        used: The amount used (bytes or seconds)
        limit: The total limit (bytes or seconds)
    ...
    try:
        ...
        # Ensure it's within bounds and properly rounded
...
def calculate_usage_percentages(subscription) -> Dict[str, Any]:
    """
    Calculate both data and time usage percentages for a subscription
    
    Args:
        subscription: Subscription object
    
    Returns:
        Dict with data and time usage percentages
    """
    data_percentage = calculate_usage_percentage(
        subscription.used_data_bytes, 
        subscription.data_limit_bytes
    )
    
    time_percentage = calculate_usage_percentage(
    ...
    return {
        ...
        'overall': max(data_percentage, time_percentage)
    }
...
def format_subscription_summary(subscription) -> Dict[str, Any]:
    ...
    """
    # Calculate usage percentages
    usage_percentages = calculate_usage_percentages(subscription)
    data_percentage = usage_percentages['data']
    time_percentage = usage_percentages['time']
    ...
    return {
        ...
        'usage': {
            ...
            'overall': usage_percentages['overall']
        },
        'activation': {
            'attempts': subscription.activation_attempts,
            'successful': subscription.activation_successful,
...
def format_subscription_statistics(subscription) -> Dict[str, Any]:
    """
    Format detailed subscription statistics for analytics
    ...
    usage_records = UsageTracking.objects.filter(subscription=subscription)
    ...
    if usage_records.exists():
        total_days = (subscription.end_date - subscription.start_date).days if subscription.end_date and subscription.start_date else 1
        total_data = usage_records.aggregate(total_data=models.Sum('data_used_bytes'))['total_data'] or 0
        total_time = usage_records.aggregate(total_time=models.Sum('session_duration_seconds'))['total_time'] or 0
        
        if total_days > 0:
            daily_data_avg = total_data / total_days
            daily_time_avg = total_time / total_days
    
    # Calculate peak usage
    peak_data = usage_records.order_by('-data_used_bytes').first()
    peak_time = usage_records.order_by('-session_duration_seconds').first()
    
    return {
        'subscription_id': str(subscription.id),
        'client_id': str(subscription.client_id),
        'total_usage_sessions': usage_records.count(),
        'total_data_used_bytes': subscription.used_data_bytes,
        'total_data_used_display': format_bytes_human_readable(subscription.used_data_bytes),
        'total_time_used_seconds': subscription.used_time_seconds,
        'total_time_used_display': format_seconds_human_readable(subscription.used_time_seconds),
        'average_daily_data_bytes': int(daily_data_avg),
        'average_daily_data_display': format_bytes_human_readable(int(daily_data_avg)),
        'average_daily_time_seconds': int(daily_time_avg),
        'average_daily_time_display': format_seconds_human_readable(int(daily_time_avg)),
        'peak_data_session': {
            'data_used_bytes': peak_data.data_used_bytes if peak_data else 0,
            'data_used_display': format_bytes_human_readable(peak_data.data_used_bytes) if peak_data else '0 B',
            'session_start': peak_data.session_start.isoformat() if peak_data else None
        } if peak_data else None,
        'peak_time_session': {
            'duration_seconds': peak_time.session_duration_seconds if peak_time else 0,
            'duration_display': format_seconds_human_readable(peak_time.session_duration_seconds) if peak_time else '0s',
            'session_start': peak_time.session_start.isoformat() if peak_time else None
        } if peak_time else None,
        'activation_success_rate': (subscription.activation_attempts / max(subscription.activation_successful, 1)) * 100 if subscription.activation_attempts > 0 else 100.0,
        'current_usage_percentage': calculate_usage_percentages(subscription)['overall']
    }
...
#         """Warm up plan caches on startup"""
#         try:
...
#         Uses LRU-like caching strategy
#         """
...
#                                 router_id: Optional[str] = None) -> Dict[str, Any]:
#         """
#         Check plan compatibility for access method and router - SIMPLIFIED
#         Only check technical compatibility, NOT auth/captive portal
#         """
...
#         # Get technical configuration ONLY
#         config = {}
...
#                 'id', 'name', 'purchases', 'price', 'category'
#             )
...
#         Uses database indexes and caching
#         """
...
#         Quick toggle for emergency overrides
#         """
#         try:
...
Core business logic for plan management with time-based availability
Production-ready with caching and algorithms
Updated: Added API exposure for compatibility check; removed update_plan_purchases (use signals); enhanced caching.
"""
...
class PlanService:
    ...
    Service for plan management operations with time variant support
    Uses caching and efficient algorithms
    Updated: Exposed compatibility check as API-friendly; optimized queries.
    """
    ...
    @classmethod
    def check_plan_compatibility_api(cls, request_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        API-exposed version of compatibility check for service_operations adapter
        Handles POST requests with plan_id, access_method, router_id
        Returns full config for seamless integration
        """
        plan_id = request_data.get('plan_id')
        access_method = request_data.get('access_method')
        router_id = request_data.get('router_id')
        
        if not plan_id or not access_method:
            return {'success': False, 'error': 'Missing required parameters'}
        
        compatibility = cls.check_plan_compatibility(plan_id, access_method, router_id)

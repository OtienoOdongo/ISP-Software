...
class PriceMatrix(models.Model):
    ...
    # Tiered/Volume Configuration - FIXED: Changed from tier_config to tier_config
    tier_config = models.JSONField(
        default=list,
        blank=True,
        help_text="Tier configuration: [{'min_qty': 1, 'price': 1000}, ...]"
    ...
    # Usage Tracking - FIXED: Changed from usage_count to usage_count
    usage_count = models.PositiveIntegerField(default=0)
    total_discount_amount = models.DecimalField(
        max_digits=12,
        decimal_places=2,
...
class DiscountRule(models.Model):
    ...
    # Eligibility Criteria - FIXED: Changed from eligibility_criteria to eligibility_criteria
    eligibility_criteria = models.JSONField(
        default=dict,
        blank=True,
        help_text="JSON criteria for eligibility"
    ...
    # Usage Limits - FIXED: Changed from max_uses_per_client to max_uses_per_client
    max_uses_per_client = models.PositiveIntegerField(null=True, blank=True)
    total_usage_limit = models.PositiveIntegerField(null=True, blank=True)
    current_usage = models.PositiveIntegerField(default=0)




# from django.db import models
# from django.core.validators import MinValueValidator, MaxValueValidator
# from django.core.exceptions import ValidationError
# from django.utils import timezone
# from decimal import Decimal
# from django.contrib.auth import get_user_model
# import uuid
# import json
# import logging
# from typing import Dict, List, Optional
# from django.db import transaction
# from network_management.utils.websocket_utils import WebSocketManager
# from network_management.models.router_management_model import Router

# logger = logging.getLogger(__name__)
# User = get_user_model()

# class PlanTemplate(models.Model):
#     CATEGORIES = (
#         ('Residential', 'Residential'),
#         ('Business', 'Business'),
#         ('Promotional', 'Promotional'),
#         ('Enterprise', 'Enterprise'),
#     )

#     name = models.CharField(max_length=100, unique=True)
#     description = models.TextField(blank=True, default="")
#     category = models.CharField(max_length=20, choices=CATEGORIES, default='Residential')
#     base_price = models.DecimalField(
#         max_digits=10, 
#         decimal_places=2, 
#         default=Decimal('0.00'),
#         validators=[MinValueValidator(Decimal('0'))]
#     )
#     access_methods = models.JSONField(default=dict)
#     is_public = models.BooleanField(default=True)
#     is_active = models.BooleanField(default=True)
#     usage_count = models.PositiveIntegerField(default=0)
#     created_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name='created_templates')
#     created_at = models.DateTimeField(auto_now_add=True)
#     updated_at = models.DateTimeField(auto_now=True)

#     def clean(self):
#         if not isinstance(self.access_methods, dict):
#             raise ValidationError({'access_methods': 'Access methods must be a dictionary'})
        
#         for method in ['hotspot', 'pppoe']:
#             if method not in self.access_methods:
#                 raise ValidationError({'access_methods': f'Missing {method} configuration'})
            
#             config = self.access_methods[method]
#             if not isinstance(config, dict):
#                 raise ValidationError({'access_methods': f'{method} configuration must be a dictionary'})
            
#             if config.get('enabled', False):
#                 required_fields = ['downloadSpeed', 'uploadSpeed', 'dataLimit', 'usageLimit']
#                 for field in required_fields:
#                     if field not in config:
#                         raise ValidationError({'access_methods': f'Missing {field} for enabled {method}'})
                    
#                     field_config = config[field]
#                     if not isinstance(field_config, dict):
#                         raise ValidationError({'access_methods': f'{method}.{field} must be a dictionary'})
                    
#                     if 'value' not in field_config:
#                         raise ValidationError({'access_methods': f'{method}.{field}.value is required'})

#         if not self.has_enabled_access_methods():
#             raise ValidationError({'access_methods': 'At least one access method must be enabled'})

#     def save(self, *args, **kwargs):
#         from network_management.utils.websocket_utils import WebSocketManager
        
#         is_new = self.pk is None
        
#         if not self.access_methods:
#             self.set_default_access_methods()
        
#         self.full_clean()
#         super().save(*args, **kwargs)
        
#         if is_new:
#             try:
#                 WebSocketManager.send_system_notification(
#                     title="New Template Created",
#                     message=f"Template '{self.name}' has been created",
#                     level="info",
#                     data={'template_id': self.id, 'template_name': self.name}
#                 )
#             except Exception as e:
#                 logger.error(f"Failed to send WebSocket notification: {e}")

#     def set_default_access_methods(self):
#         self.access_methods = {
#             'hotspot': {
#                 'enabled': True,
#                 'downloadSpeed': {'value': '10', 'unit': 'Mbps'},
#                 'uploadSpeed': {'value': '5', 'unit': 'Mbps'},
#                 'dataLimit': {'value': '10', 'unit': 'GB'},
#                 'usageLimit': {'value': '24', 'unit': 'Hours'},
#                 'bandwidthLimit': 0,
#                 'maxDevices': 1,
#                 'sessionTimeout': 86400,
#                 'idleTimeout': 300,
#                 'validityPeriod': {'value': '30', 'unit': 'Days'},
#                 'macBinding': False,
#             },
#             'pppoe': {
#                 'enabled': False,
#                 'downloadSpeed': {'value': '10', 'unit': 'Mbps'},
#                 'uploadSpeed': {'value': '5', 'unit': 'Mbps'},
#                 'dataLimit': {'value': '10', 'unit': 'GB'},
#                 'usageLimit': {'value': '24', 'unit': 'Hours'},
#                 'bandwidthLimit': 0,
#                 'maxDevices': 1,
#                 'sessionTimeout': 86400,
#                 'idleTimeout': 300,
#                 'validityPeriod': {'value': '30', 'unit': 'Days'},
#                 'macBinding': False,
#                 'ipPool': 'pppoe-pool-1',
#                 'serviceName': '',
#                 'mtu': 1492,
#                 'dnsServers': ['8.8.8.8', '1.1.1.1'],
#             }
#         }

#     def increment_usage(self):
#         with transaction.atomic():
#             PlanTemplate.objects.filter(pk=self.pk).update(
#                 usage_count=models.F('usage_count') + 1
#             )
#             self.refresh_from_db()

#     def has_enabled_access_methods(self):
#         return any(
#             method_config.get('enabled', False) 
#             for method_config in self.access_methods.values()
#         )

#     def get_enabled_access_methods(self):
#         return [
#             method for method, config in self.access_methods.items() 
#             if config.get('enabled', False)
#         ]

#     def get_access_type(self):
#         enabled_methods = self.get_enabled_access_methods()
#         if 'hotspot' in enabled_methods and 'pppoe' in enabled_methods:
#             return 'both'
#         elif 'hotspot' in enabled_methods:
#             return 'hotspot'
#         elif 'pppoe' in enabled_methods:
#             return 'pppoe'
#         return None

#     def __str__(self):
#         return f"{self.name} Template - {self.category}"

#     class Meta:
#         ordering = ['name']
#         verbose_name = 'Plan Template'
#         verbose_name_plural = 'Plan Templates'
#         indexes = [
#             models.Index(fields=['name']),
#             models.Index(fields=['category']),
#             models.Index(fields=['is_public']),
#             models.Index(fields=['is_active']),
#             models.Index(fields=['created_at']),
#         ]


# class InternetPlan(models.Model):
#     PLAN_TYPES = (
#         ('Paid', 'Paid'),
#         ('Free Trial', 'Free Trial'),
#     )
    
#     CATEGORIES = (
#         ('Residential', 'Residential'),
#         ('Business', 'Business'),
#         ('Promotional', 'Promotional'),
#         ('Enterprise', 'Enterprise'),
#     )
    
#     PRIORITY_LEVELS = (
#         (1, 'Lowest'),
#         (2, 'Low'),
#         (3, 'Medium'),
#         (4, 'High'),
#         (5, 'Highest'),
#         (6, 'Critical'),
#         (7, 'Premium'),
#         (8, 'VIP'),
#     )

#     plan_type = models.CharField(max_length=20, choices=PLAN_TYPES, default='Paid')
#     name = models.CharField(max_length=100, unique=True)
#     price = models.DecimalField(
#         max_digits=10, 
#         decimal_places=2, 
#         default=Decimal('0.00'),
#         validators=[MinValueValidator(Decimal('0'))]
#     )
#     active = models.BooleanField(default=True)
#     category = models.CharField(max_length=20, choices=CATEGORIES, default='Residential')
#     description = models.TextField(blank=True, default="")
#     template = models.ForeignKey(
#         PlanTemplate, 
#         on_delete=models.SET_NULL, 
#         null=True, 
#         blank=True, 
#         related_name='created_plans'
#     )
#     access_methods = models.JSONField(default=dict)
#     priority_level = models.IntegerField(choices=PRIORITY_LEVELS, default=4)
#     router_specific = models.BooleanField(default=False)
#     allowed_routers = models.ManyToManyField(
#         'network_management.Router',
#         blank=True,
#         related_name='allowed_plans'
#     )
#     FUP_policy = models.TextField(blank=True, default="")
#     FUP_threshold = models.PositiveIntegerField(
#         default=80,
#         validators=[MinValueValidator(1), MaxValueValidator(100)]
#     )
#     purchases = models.PositiveIntegerField(default=0)
#     created_at = models.DateTimeField(auto_now_add=True)
#     updated_at = models.DateTimeField(auto_now=True)
#     client_sessions = models.JSONField(default=dict, blank=True)

#     def clean(self):
#         if self.plan_type == 'Free Trial':
#             if self.price != Decimal('0'):
#                 raise ValidationError({'price': 'Free Trial plans must have price set to 0'})
#             if self.router_specific:
#                 raise ValidationError({'router_specific': 'Free Trial plans cannot be router-specific'})
#             if self.priority_level > 4:
#                 raise ValidationError({'priority_level': 'Free Trial plans cannot have premium priority levels'})

#         if not isinstance(self.access_methods, dict):
#             raise ValidationError({'access_methods': 'Access methods must be a dictionary'})
        
#         for method in ['hotspot', 'pppoe']:
#             if method not in self.access_methods:
#                 raise ValidationError({'access_methods': f'Missing {method} configuration'})
            
#             config = self.access_methods[method]
#             if not isinstance(config, dict):
#                 raise ValidationError({'access_methods': f'{method} configuration must be a dictionary'})
            
#             if config.get('enabled', False):
#                 required_fields = ['downloadSpeed', 'uploadSpeed', 'dataLimit', 'usageLimit']
#                 for field in required_fields:
#                     if field not in config:
#                         raise ValidationError({'access_methods': f'Missing {field} for enabled {method}'})
                    
#                     field_config = config[field]
#                     if not isinstance(field_config, dict):
#                         raise ValidationError({'access_methods': f'{method}.{field} must be a dictionary'})
                    
#                     if 'value' not in field_config:
#                         raise ValidationError({'access_methods': f'{method}.{field}.value is required'})

#         if not self.has_enabled_access_methods():
#             raise ValidationError({'access_methods': 'At least one access method must be enabled'})

#     def save(self, *args, **kwargs):
#         from network_management.utils.websocket_utils import WebSocketManager
        
#         is_new = self.pk is None
        
#         if not self.access_methods:
#             self.set_default_access_methods()
        
#         self.full_clean()
#         super().save(*args, **kwargs)
        
#         try:
#             if is_new:
#                 WebSocketManager.send_system_notification(
#                     title="New Plan Created",
#                     message=f"Plan '{self.name}' has been created",
#                     level="info",
#                     data={'plan_id': self.id, 'plan_name': self.name}
#                 )
#             else:
#                 WebSocketManager.send_system_notification(
#                     title="Plan Updated", 
#                     message=f"Plan '{self.name}' has been updated",
#                     level="info",
#                     data={'plan_id': self.id, 'plan_name': self.name}
#                 )
#         except Exception as e:
#             logger.error(f"Failed to send WebSocket notification: {e}")

#     def set_default_access_methods(self):
#         self.access_methods = {
#             'hotspot': {
#                 'enabled': True,
#                 'downloadSpeed': {'value': '10', 'unit': 'Mbps'},
#                 'uploadSpeed': {'value': '5', 'unit': 'Mbps'},
#                 'dataLimit': {'value': '10', 'unit': 'GB'},
#                 'usageLimit': {'value': '24', 'unit': 'Hours'},
#                 'bandwidthLimit': 0,
#                 'maxDevices': 1,
#                 'sessionTimeout': 86400,
#                 'idleTimeout': 300,
#                 'validityPeriod': {'value': '30', 'unit': 'Days'},
#                 'macBinding': False,
#             },
#             'pppoe': {
#                 'enabled': False,
#                 'downloadSpeed': {'value': '10', 'unit': 'Mbps'},
#                 'uploadSpeed': {'value': '5', 'unit': 'Mbps'},
#                 'dataLimit': {'value': '10', 'unit': 'GB'},
#                 'usageLimit': {'value': '24', 'unit': 'Hours'},
#                 'bandwidthLimit': 0,
#                 'maxDevices': 1,
#                 'sessionTimeout': 86400,
#                 'idleTimeout': 300,
#                 'validityPeriod': {'value': '30', 'unit': 'Days'},
#                 'macBinding': False,
#                 'ipPool': 'pppoe-pool-1',
#                 'serviceName': '',
#                 'mtu': 1492,
#                 'dnsServers': ['8.8.8.8', '1.1.1.1'],
#             }
#         }

#     def create_from_template(self, template):
#         self.name = template.name
#         self.category = template.category
#         self.price = template.base_price
#         self.description = template.description
#         self.access_methods = template.access_methods
#         self.template = template
        
#         if template.base_price > 0:
#             self.plan_type = 'Paid'
#         else:
#             self.plan_type = 'Free Trial'
        
#         template.increment_usage()

#     def has_enabled_access_methods(self):
#         return any(
#             method_config.get('enabled', False) 
#             for method_config in self.access_methods.values()
#         )

#     def get_enabled_access_methods(self):
#         return [
#             method for method, config in self.access_methods.items() 
#             if config.get('enabled', False)
#         ]

#     def get_access_type(self):
#         enabled_methods = self.get_enabled_access_methods()
#         if 'hotspot' in enabled_methods and 'pppoe' in enabled_methods:
#             return 'both'
#         elif 'hotspot' in enabled_methods:
#             return 'hotspot'
#         elif 'pppoe' in enabled_methods:
#             return 'pppoe'
#         return None

#     def can_be_used_on_router(self, router_id):
#         if not self.router_specific:
#             return True
        
#         from network_management.models.router_management_model import Router
#         return self.allowed_routers.filter(id=router_id).exists()

#     def increment_purchases(self):
#         self.purchases += 1
#         self.save(update_fields=['purchases'])

#     def get_router_compatibility(self):
#         from network_management.models.router_management_model import Router
        
#         routers = Router.objects.filter(is_active=True, status='connected')
#         compatibility_data = []
        
#         for router in routers:
#             compatible = self.can_be_used_on_router(router.id)
#             compatibility_data.append({
#                 'router_id': router.id,
#                 'router_name': router.name,
#                 'compatible': compatible,
#                 'access_methods': self.get_enabled_access_methods() if compatible else []
#             })
        
#         return compatibility_data

#     def __str__(self):
#         return f"{self.name} ({self.plan_type}) - {self.price} Ksh"

#     class Meta:
#         ordering = ['name']
#         verbose_name = 'Internet Plan'
#         verbose_name_plural = 'Internet Plans'
#         indexes = [
#             models.Index(fields=['name']),
#             models.Index(fields=['category']),
#             models.Index(fields=['plan_type']),
#             models.Index(fields=['active']),
#             models.Index(fields=['price']),
#             models.Index(fields=['created_at']),
#             models.Index(fields=['template']),
#         ]


# class Subscription(models.Model):
#     STATUS_CHOICES = (
#         ('active', 'Active'),
#         ('expired', 'Expired'),
#         ('suspended', 'Suspended'),
#         ('cancelled', 'Cancelled'),
#     )

#     ACCESS_METHOD_CHOICES = (
#         ('hotspot', 'Hotspot'),
#         ('pppoe', 'PPPoE'),
#     )

#     client = models.ForeignKey('account.Client', on_delete=models.CASCADE, related_name='subscriptions')
#     internet_plan = models.ForeignKey(InternetPlan, on_delete=models.SET_NULL, null=True, blank=True, related_name='subscriptions')
#     router = models.ForeignKey('network_management.Router', on_delete=models.SET_NULL, null=True, blank=True, related_name='subscriptions')
#     transaction_reference = models.CharField(max_length=100, unique=True, null=True, blank=True)  
#     access_method = models.CharField(max_length=10, choices=ACCESS_METHOD_CHOICES, default='hotspot')
#     status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='active')
#     mac_address = models.CharField(max_length=17, blank=True, null=True)
#     start_date = models.DateTimeField(auto_now_add=True)
#     end_date = models.DateTimeField()
#     remaining_data = models.BigIntegerField(default=0)
#     remaining_time = models.PositiveIntegerField(default=0)
#     data_used = models.BigIntegerField(default=0)
#     time_used = models.PositiveIntegerField(default=0)
#     last_activity = models.DateTimeField(auto_now=True)
#     is_active = models.BooleanField(default=True)
#     activation_attempts = models.PositiveIntegerField(default=0)
#     last_activation_attempt = models.DateTimeField(null=True, blank=True)
#     activation_successful = models.BooleanField(default=False)
#     activation_error = models.TextField(blank=True, null=True)

#     def clean(self):
#         if self.end_date <= timezone.now():
#             raise ValidationError("End date must be in the future.")
        
#         if self.internet_plan:
#             access_methods = self.internet_plan.get_enabled_access_methods()
#             if self.access_method not in access_methods:
#                 raise ValidationError(f"Selected access method '{self.access_method}' is not enabled for this plan")

#     def save(self, *args, **kwargs):
#         from network_management.utils.websocket_utils import WebSocketManager
        
#         is_new = self.pk is None
#         super().save(*args, **kwargs)
        
#         if is_new and self.status == 'active':
#             try:
#                 WebSocketManager.send_system_notification(
#                     title="New Subscription",
#                     message=f"New {self.access_method} subscription activated for {self.client.user.username if self.client else 'Unknown'}",
#                     level="info",
#                     data={
#                         'subscription_id': self.id,
#                         'client_id': self.client.id if self.client else None,
#                         'plan_name': self.internet_plan.name if self.internet_plan else 'Unknown'
#                     }
#                 )
#             except Exception as e:
#                 logger.error(f"Failed to send WebSocket notification: {e}")

#     def activate_on_router(self):
#         from network_management.utils.websocket_utils import WebSocketManager
        
#         if not self.router or not self.client:
#             return False, "Missing router or client information"
        
#         if self.router.status != 'connected':
#             return False, f"Router {self.router.name} is not connected"
        
#         self.activation_attempts += 1
#         self.last_activation_attempt = timezone.now()
        
#         try:
#             success, message = self._perform_router_activation()
            
#             if success:
#                 self.activation_successful = True
#                 self.activation_error = None
#                 self.save()
                
#                 WebSocketManager.send_system_notification(
#                     title="Subscription Activated",
#                     message=f"Subscription for {self.client.user.username if self.client else 'Unknown'} activated on {self.router.name}",
#                     level="success"
#                 )
                
#                 return True, "Subscription activated successfully"
#             else:
#                 self.activation_successful = False
#                 self.activation_error = message
#                 self.save()
#                 return False, f"Failed to activate on router: {message}"
                
#         except Exception as e:
#             self.activation_successful = False
#             self.activation_error = str(e)
#             self.save()
#             return False, f"Activation error: {str(e)}"
    
#     def _perform_router_activation(self):
#         try:
#             if self.access_method == 'hotspot':
#                 return self._activate_hotspot_on_router()
#             elif self.access_method == 'pppoe':
#                 return self._activate_pppoe_on_router()
#             else:
#                 return False, f"Unsupported access method: {self.access_method}"
#         except Exception as e:
#             logger.error(f"Router activation failed: {e}")
#             return False, str(e)

#     def _activate_hotspot_on_router(self):
#         try:
#             if self.router.type == "mikrotik":
#                 from routeros_api import RouterOsApiPool
                
#                 api_pool = RouterOsApiPool(
#                     self.router.ip,
#                     username=self.router.username,
#                     password=self.router.password,
#                     port=self.router.port,
#                     plaintext_login=True,
#                     timeout=10
#                 )
                
#                 api = api_pool.get_api()
                
#                 method_config = self.internet_plan.access_methods.get('hotspot', {})
#                 data_limit = 0
                
#                 if method_config.get('dataLimit', {}).get('value', '').lower() != 'unlimited':
#                     try:
#                         data_value = method_config['dataLimit']['value']
#                         data_unit = method_config['dataLimit']['unit']
#                         multiplier = 1024 ** 3 if data_unit == "GB" else 1024 ** 2
#                         data_limit = int(float(data_value) * multiplier)
#                     except (ValueError, KeyError):
#                         data_limit = 0

#                 username = self.client.user.username if self.client and self.client.user else f"user_{self.client.id}"
                
#                 hotspot_resource = api.get_resource("/ip/hotspot/user")
#                 existing_users = hotspot_resource.get(name=username)
                
#                 if existing_users:
#                     hotspot_resource.set(
#                         id=existing_users[0]['id'],
#                         password=str(self.client.id),
#                         profile=method_config.get('profile', 'default'),
#                         limit_bytes_total=str(data_limit)
#                     )
#                 else:
#                     hotspot_resource.add(
#                         name=username,
#                         password=str(self.client.id),
#                         profile=method_config.get('profile', 'default'),
#                         limit_bytes_total=str(data_limit)
#                     )

#                 if self.remaining_time > 0:
#                     active_resource = api.get_resource("/ip/hotspot/active")
#                     active_sessions = active_resource.get(mac_address=self.mac_address.lower()) if self.mac_address else []
                    
#                     for session in active_sessions:
#                         active_resource.set(
#                             id=session['id'],
#                             idle_timeout=f"{max(1, self.remaining_time // 60)}m"
#                         )

#                 api_pool.disconnect()
#                 return True, "Hotspot user configured successfully"

#             elif self.router.type == "ubiquiti":
#                 import requests
#                 import urllib3
#                 urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
                
#                 controller_url = f"https://{self.router.ip}:{self.router.port}/api/s/default/cmd/stamgr"
                
#                 method_config = self.internet_plan.access_methods.get('hotspot', {})
#                 auth_minutes = max(1, self.remaining_time // 60) if self.remaining_time > 0 else 1440
                
#                 data_limit = 0
#                 if method_config.get('dataLimit', {}).get('value', '').lower() != 'unlimited':
#                     try:
#                         data_value = method_config['dataLimit']['value']
#                         data_unit = method_config['dataLimit']['unit']
#                         multiplier = 1024 ** 3 if data_unit == "GB" else 1024 ** 2
#                         data_limit = int(float(data_value) * multiplier)
#                     except (ValueError, KeyError):
#                         data_limit = 0

#                 data = {
#                     "cmd": "authorize-guest",
#                     "mac": self.mac_address.lower() if self.mac_address else "",
#                     "minutes": auth_minutes,
#                     "up": int(method_config.get('uploadSpeed', {}).get('value', 0)),
#                     "down": int(method_config.get('downloadSpeed', {}).get('value', 0)),
#                     "bytes": data_limit
#                 }
                
#                 response = requests.post(
#                     controller_url,
#                     json=data,
#                     auth=(self.router.username, self.router.password),
#                     verify=False,
#                     timeout=10
#                 )
                
#                 if response.status_code == 200:
#                     return True, "Hotspot user authorized successfully"
#                 else:
#                     return False, f"Ubiquiti API error: {response.status_code} - {response.text}"

#             else:
#                 return False, f"Unsupported router type for hotspot: {self.router.type}"

#         except Exception as e:
#             logger.error(f"Hotspot activation failed: {e}")
#             return False, str(e)

#     def _activate_pppoe_on_router(self):
#         try:
#             if self.router.type == "mikrotik":
#                 from routeros_api import RouterOsApiPool
                
#                 api_pool = RouterOsApiPool(
#                     self.router.ip,
#                     username=self.router.username,
#                     password=self.router.password,
#                     port=self.router.port,
#                     plaintext_login=True,
#                     timeout=10
#                 )
                
#                 api = api_pool.get_api()
                
#                 method_config = self.internet_plan.access_methods.get('pppoe', {})
#                 username = self.client.user.username if self.client and self.client.user else f"user_{self.client.id}"
                
#                 pppoe_secret = api.get_resource("/ppp/secret")
#                 existing_secrets = pppoe_secret.get(name=username)
                
#                 if existing_secrets:
#                     pppoe_secret.set(
#                         id=existing_secrets[0]['id'],
#                         password=str(self.client.id),
#                         service="pppoe",
#                         profile=method_config.get('profile', 'default'),
#                         remote_address=method_config.get('ipPool', 'dynamic')
#                     )
#                 else:
#                     pppoe_secret.add(
#                         name=username,
#                         password=str(self.client.id),
#                         service="pppoe",
#                         profile=method_config.get('profile', 'default'),
#                         remote_address=method_config.get('ipPool', 'dynamic')
#                     )

#                 if self.remaining_time > 0:
#                     profile_resource = api.get_resource("/ppp/profile")
#                     existing_profiles = profile_resource.get(name=method_config.get('profile', 'default'))
                    
#                     if existing_profiles:
#                         profile_resource.set(
#                             id=existing_profiles[0]['id'],
#                             session_timeout=f"{max(1, self.remaining_time // 60)}m"
#                         )

#                 api_pool.disconnect()
#                 return True, "PPPoE user configured successfully"

#             else:
#                 return False, f"PPPoE not supported on router type: {self.router.type}"

#         except Exception as e:
#             logger.error(f"PPPoE activation failed: {e}")
#             return False, str(e)

#     def update_usage(self, data_used, time_used):
#         self.data_used += data_used
#         self.time_used += time_used
        
#         if self.internet_plan:
#             method_config = self.internet_plan.access_methods.get(self.access_method, {})
            
#             data_limit = method_config.get('dataLimit', {})
#             if data_limit.get('value', '').lower() != 'unlimited':
#                 self.remaining_data -= data_used
#                 if self.remaining_data < 0:
#                     self.remaining_data = 0
#                     self.status = 'suspended'
            
#             usage_limit = method_config.get('usageLimit', {})
#             if usage_limit.get('value', '').lower() != 'unlimited':
#                 self.remaining_time -= time_used
#                 if self.remaining_time < 0:
#                     self.remaining_time = 0
#                     self.status = 'suspended'
        
#         self.last_activity = timezone.now()
#         self.save()

#     def get_remaining_data_display(self):
#         if self.internet_plan:
#             method_config = self.internet_plan.access_methods.get(self.access_method, {})
#             data_limit = method_config.get('dataLimit', {})
#             if data_limit.get('value', '').lower() == 'unlimited':
#                 return "Unlimited"
        
#         bytes_val = self.remaining_data
#         if bytes_val >= 1024 ** 3:
#             return f"{(bytes_val / (1024 ** 3)):.2f} GB"
#         elif bytes_val >= 1024 ** 2:
#             return f"{(bytes_val / (1024 ** 2)):.2f} MB"
#         elif bytes_val >= 1024:
#             return f"{(bytes_val / 1024):.2f} KB"
#         else:
#             return f"{bytes_val} B"

#     def get_remaining_time_display(self):
#         if self.internet_plan:
#             method_config = self.internet_plan.access_methods.get(self.access_method, {})
#             usage_limit = method_config.get('usageLimit', {})
#             if usage_limit.get('value', '').lower() == 'unlimited':
#                 return "Unlimited"
        
#         seconds = self.remaining_time
#         if seconds >= 3600:
#             hours = seconds // 3600
#             minutes = (seconds % 3600) // 60
#             return f"{hours}h {minutes}m"
#         elif seconds >= 60:
#             minutes = seconds // 60
#             return f"{minutes}m"
#         else:
#             return f"{seconds}s"

#     def __str__(self):
#         return f"Subscription for {self.client.user.username if self.client else 'Unknown'} - {self.internet_plan.name if self.internet_plan else 'No Plan'} ({self.access_method})"

#     class Meta:
#         verbose_name = 'Subscription'
#         verbose_name_plural = 'Subscriptions'
#         ordering = ['-start_date']
#         indexes = [
#             models.Index(fields=['client', 'start_date']),
#             models.Index(fields=['internet_plan']),
#             models.Index(fields=['is_active']),
#             models.Index(fields=['status']),
#             models.Index(fields=['mac_address']),
#             models.Index(fields=['router']),
#             models.Index(fields=['access_method']),
#             models.Index(fields=['transaction_reference']),  
#         ]










from django.db import models
from django.core.validators import MinValueValidator, MaxValueValidator
from django.core.exceptions import ValidationError
from django.utils import timezone
from decimal import Decimal
from django.contrib.auth import get_user_model
import uuid
import logging
from typing import Dict, List, Optional
from django.db import transaction

logger = logging.getLogger(__name__)
User = get_user_model()

class PlanTemplate(models.Model):
    CATEGORIES = (
        ('Residential', 'Residential'),
        ('Business', 'Business'),
        ('Promotional', 'Promotional'),
        ('Enterprise', 'Enterprise'),
    )

    name = models.CharField(max_length=100, unique=True)
    description = models.TextField(blank=True, default="")
    category = models.CharField(max_length=20, choices=CATEGORIES, default='Residential')
    base_price = models.DecimalField(
        max_digits=10, 
        decimal_places=2, 
        default=Decimal('0.00'),
        validators=[MinValueValidator(Decimal('0'))]
    )
    access_methods = models.JSONField(default=dict)
    is_public = models.BooleanField(default=True)
    is_active = models.BooleanField(default=True)
    usage_count = models.PositiveIntegerField(default=0)
    created_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name='created_templates')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

    def clean(self):
        if not isinstance(self.access_methods, dict):
            raise ValidationError({'access_methods': 'Access methods must be a dictionary'})
        
        for method in ['hotspot', 'pppoe']:
            if method not in self.access_methods:
                raise ValidationError({'access_methods': f'Missing {method} configuration'})
            
            config = self.access_methods[method]
            if not isinstance(config, dict):
                raise ValidationError({'access_methods': f'{method} configuration must be a dictionary'})
            
            if config.get('enabled', False):
                required_fields = ['downloadSpeed', 'uploadSpeed', 'dataLimit', 'usageLimit']
                for field in required_fields:
                    if field not in config:
                        raise ValidationError({'access_methods': f'Missing {field} for enabled {method}'})
                    
                    field_config = config[field]
                    if not isinstance(field_config, dict):
                        raise ValidationError({'access_methods': f'{method}.{field} must be a dictionary'})
                    
                    if 'value' not in field_config:
                        raise ValidationError({'access_methods': f'{method}.{field}.value is required'})

        if not self.has_enabled_access_methods():
            raise ValidationError({'access_methods': 'At least one access method must be enabled'})

    def save(self, *args, **kwargs):
        if not self.access_methods:
            self.set_default_access_methods()
        
        self.full_clean()
        super().save(*args, **kwargs)

    def set_default_access_methods(self):
        self.access_methods = {
            'hotspot': {
                'enabled': True,
                'downloadSpeed': {'value': '10', 'unit': 'Mbps'},
                'uploadSpeed': {'value': '5', 'unit': 'Mbps'},
                'dataLimit': {'value': '10', 'unit': 'GB'},
                'usageLimit': {'value': '24', 'unit': 'Hours'},
                'bandwidthLimit': 0,
                'maxDevices': 1,
                'sessionTimeout': 86400,
                'idleTimeout': 300,
                'validityPeriod': {'value': '30', 'unit': 'Days'},
                'macBinding': False,
            },
            'pppoe': {
                'enabled': False,
                'downloadSpeed': {'value': '10', 'unit': 'Mbps'},
                'uploadSpeed': {'value': '5', 'unit': 'Mbps'},
                'dataLimit': {'value': '10', 'unit': 'GB'},
                'usageLimit': {'value': '24', 'unit': 'Hours'},
                'bandwidthLimit': 0,
                'maxDevices': 1,
                'sessionTimeout': 86400,
                'idleTimeout': 300,
                'validityPeriod': {'value': '30', 'unit': 'Days'},
                'macBinding': False,
                'ipPool': 'pppoe-pool-1',
                'serviceName': '',
                'mtu': 1492,
                'dnsServers': ['8.8.8.8', '1.1.1.1'],
            }
        }

    def increment_usage(self):
        with transaction.atomic():
            PlanTemplate.objects.filter(pk=self.pk).update(
                usage_count=models.F('usage_count') + 1
            )
            self.refresh_from_db()

    def has_enabled_access_methods(self):
        return any(
            method_config.get('enabled', False) 
            for method_config in self.access_methods.values()
        )

    def get_enabled_access_methods(self):
        return [
            method for method, config in self.access_methods.items() 
            if config.get('enabled', False)
        ]

    def get_access_type(self):
        enabled_methods = self.get_enabled_access_methods()
        if 'hotspot' in enabled_methods and 'pppoe' in enabled_methods:
            return 'both'
        elif 'hotspot' in enabled_methods:
            return 'hotspot'
        elif 'pppoe' in enabled_methods:
            return 'pppoe'
        return None

    def __str__(self):
        return f"{self.name} Template - {self.category}"

    class Meta:
        ordering = ['name']
        verbose_name = 'Plan Template'
        verbose_name_plural = 'Plan Templates'
        indexes = [
            models.Index(fields=['name']),
            models.Index(fields=['category']),
            models.Index(fields=['is_public']),
            models.Index(fields=['is_active']),
            models.Index(fields=['created_at']),
        ]


class InternetPlan(models.Model):
    PLAN_TYPES = (
        ('Paid', 'Paid'),
        ('Free Trial', 'Free Trial'),
    )
    
    CATEGORIES = (
        ('Residential', 'Residential'),
        ('Business', 'Business'),
        ('Promotional', 'Promotional'),
        ('Enterprise', 'Enterprise'),
    )
    
    PRIORITY_LEVELS = (
        (1, 'Lowest'),
        (2, 'Low'),
        (3, 'Medium'),
        (4, 'High'),
        (5, 'Highest'),
        (6, 'Critical'),
        (7, 'Premium'),
        (8, 'VIP'),
    )

    plan_type = models.CharField(max_length=20, choices=PLAN_TYPES, default='Paid')
    name = models.CharField(max_length=100, unique=True)
    price = models.DecimalField(
        max_digits=10, 
        decimal_places=2, 
        default=Decimal('0.00'),
        validators=[MinValueValidator(Decimal('0'))]
    )
    active = models.BooleanField(default=True)
    category = models.CharField(max_length=20, choices=CATEGORIES, default='Residential')
    description = models.TextField(blank=True, default="")
    template = models.ForeignKey(
        PlanTemplate, 
        on_delete=models.SET_NULL, 
        null=True, 
        blank=True, 
        related_name='created_plans'
    )
    access_methods = models.JSONField(default=dict)
    priority_level = models.IntegerField(choices=PRIORITY_LEVELS, default=4)
    router_specific = models.BooleanField(default=False)
    allowed_routers = models.ManyToManyField(
        'network_management.Router',
        blank=True,
        related_name='allowed_plans'
    )
    FUP_policy = models.TextField(blank=True, default="")
    FUP_threshold = models.PositiveIntegerField(
        default=80,
        validators=[MinValueValidator(1), MaxValueValidator(100)]
    )
    purchases = models.PositiveIntegerField(default=0)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    client_sessions = models.JSONField(default=dict, blank=True)

    def clean(self):
        if self.plan_type == 'Free Trial':
            if self.price != Decimal('0'):
                raise ValidationError({'price': 'Free Trial plans must have price set to 0'})
            if self.router_specific:
                raise ValidationError({'router_specific': 'Free Trial plans cannot be router-specific'})
            if self.priority_level > 4:
                raise ValidationError({'priority_level': 'Free Trial plans cannot have premium priority levels'})

        if not isinstance(self.access_methods, dict):
            raise ValidationError({'access_methods': 'Access methods must be a dictionary'})
        
        for method in ['hotspot', 'pppoe']:
            if method not in self.access_methods:
                raise ValidationError({'access_methods': f'Missing {method} configuration'})
            
            config = self.access_methods[method]
            if not isinstance(config, dict):
                raise ValidationError({'access_methods': f'{method} configuration must be a dictionary'})
            
            if config.get('enabled', False):
                required_fields = ['downloadSpeed', 'uploadSpeed', 'dataLimit', 'usageLimit']
                for field in required_fields:
                    if field not in config:
                        raise ValidationError({'access_methods': f'Missing {field} for enabled {method}'})
                    
                    field_config = config[field]
                    if not isinstance(field_config, dict):
                        raise ValidationError({'access_methods': f'{method}.{field} must be a dictionary'})
                    
                    if 'value' not in field_config:
                        raise ValidationError({'access_methods': f'{method}.{field}.value is required'})

        if not self.has_enabled_access_methods():
            raise ValidationError({'access_methods': 'At least one access method must be enabled'})

    def save(self, *args, **kwargs):
        if not self.access_methods:
            self.set_default_access_methods()
        
        self.full_clean()
        super().save(*args, **kwargs)

    def set_default_access_methods(self):
        self.access_methods = {
            'hotspot': {
                'enabled': True,
                'downloadSpeed': {'value': '10', 'unit': 'Mbps'},
                'uploadSpeed': {'value': '5', 'unit': 'Mbps'},
                'dataLimit': {'value': '10', 'unit': 'GB'},
                'usageLimit': {'value': '24', 'unit': 'Hours'},
                'bandwidthLimit': 0,
                'maxDevices': 1,
                'sessionTimeout': 86400,
                'idleTimeout': 300,
                'validityPeriod': {'value': '30', 'unit': 'Days'},
                'macBinding': False,
            },
            'pppoe': {
                'enabled': False,
                'downloadSpeed': {'value': '10', 'unit': 'Mbps'},
                'uploadSpeed': {'value': '5', 'unit': 'Mbps'},
                'dataLimit': {'value': '10', 'unit': 'GB'},
                'usageLimit': {'value': '24', 'unit': 'Hours'},
                'bandwidthLimit': 0,
                'maxDevices': 1,
                'sessionTimeout': 86400,
                'idleTimeout': 300,
                'validityPeriod': {'value': '30', 'unit': 'Days'},
                'macBinding': False,
                'ipPool': 'pppoe-pool-1',
                'serviceName': '',
                'mtu': 1492,
                'dnsServers': ['8.8.8.8', '1.1.1.1'],
            }
        }

    def create_from_template(self, template):
        self.name = template.name
        self.category = template.category
        self.price = template.base_price
        self.description = template.description
        self.access_methods = template.access_methods
        self.template = template
        
        if template.base_price > 0:
            self.plan_type = 'Paid'
        else:
            self.plan_type = 'Free Trial'
        
        template.increment_usage()

    def has_enabled_access_methods(self):
        return any(
            method_config.get('enabled', False) 
            for method_config in self.access_methods.values()
        )

    def get_enabled_access_methods(self):
        return [
            method for method, config in self.access_methods.items() 
            if config.get('enabled', False)
        ]

    def get_access_type(self):
        enabled_methods = self.get_enabled_access_methods()
        if 'hotspot' in enabled_methods and 'pppoe' in enabled_methods:
            return 'both'
        elif 'hotspot' in enabled_methods:
            return 'hotspot'
        elif 'pppoe' in enabled_methods:
            return 'pppoe'
        return None

    def can_be_used_on_router(self, router_id):
        if not self.router_specific:
            return True
        
        from network_management.models.router_management_model import Router
        return self.allowed_routers.filter(id=router_id).exists()

    def increment_purchases(self):
        self.purchases += 1
        self.save(update_fields=['purchases'])

    def get_router_compatibility(self):
        from network_management.models.router_management_model import Router
        
        routers = Router.objects.filter(is_active=True, status='connected')
        compatibility_data = []
        
        for router in routers:
            compatible = self.can_be_used_on_router(router.id)
            compatibility_data.append({
                'router_id': router.id,
                'router_name': router.name,
                'compatible': compatible,
                'access_methods': self.get_enabled_access_methods() if compatible else []
            })
        
        return compatibility_data

    def __str__(self):
        return f"{self.name} ({self.plan_type}) - {self.price} Ksh"

    class Meta:
        ordering = ['name']
        verbose_name = 'Internet Plan'
        verbose_name_plural = 'Internet Plans'
        indexes = [
            models.Index(fields=['name']),
            models.Index(fields=['category']),
            models.Index(fields=['plan_type']),
            models.Index(fields=['active']),
            models.Index(fields=['price']),
            models.Index(fields=['created_at']),
            models.Index(fields=['template']),
        ]


class Subscription(models.Model):
    STATUS_CHOICES = (
        ('pending', 'Pending Payment'),
        ('active', 'Active'),
        ('expired', 'Expired'),
        ('suspended', 'Suspended'),
        ('cancelled', 'Cancelled'),
    )

    ACCESS_METHOD_CHOICES = (
        ('hotspot', 'Hotspot'),
        ('pppoe', 'PPPoE'),
    )

    client = models.ForeignKey('account.Client', on_delete=models.CASCADE, related_name='subscriptions')
    internet_plan = models.ForeignKey(InternetPlan, on_delete=models.SET_NULL, null=True, blank=True, related_name='subscriptions')
    router = models.ForeignKey('network_management.Router', on_delete=models.SET_NULL, null=True, blank=True, related_name='subscriptions')
    transaction_reference = models.CharField(max_length=100, unique=True, null=True, blank=True)  
    access_method = models.CharField(max_length=10, choices=ACCESS_METHOD_CHOICES, default='hotspot')
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='pending')
    mac_address = models.CharField(max_length=17, blank=True, null=True)
    start_date = models.DateTimeField(auto_now_add=True)
    end_date = models.DateTimeField()
    remaining_data = models.BigIntegerField(default=0)
    remaining_time = models.PositiveIntegerField(default=0)
    data_used = models.BigIntegerField(default=0)
    time_used = models.PositiveIntegerField(default=0)
    last_activity = models.DateTimeField(auto_now=True)
    is_active = models.BooleanField(default=True)
    
    # Activation tracking (moved to service layer)
    activation_requested = models.BooleanField(default=False)
    activation_successful = models.BooleanField(default=False)
    activation_error = models.TextField(blank=True, null=True)
    activation_requested_at = models.DateTimeField(null=True, blank=True)

    def clean(self):
        if self.end_date <= timezone.now():
            raise ValidationError("End date must be in the future.")
        
        if self.internet_plan:
            access_methods = self.internet_plan.get_enabled_access_methods()
            if self.access_method not in access_methods:
                raise ValidationError(f"Selected access method '{self.access_method}' is not enabled for this plan")

    def save(self, *args, **kwargs):
        super().save(*args, **kwargs)

    def request_activation(self):
        """Request activation via NetworkManagement service"""
        from internet_plans.services.activation_service import ActivationService
        
        return ActivationService.request_subscription_activation(self)

    def update_usage(self, data_used, time_used):
        """Update usage statistics"""
        self.data_used += data_used
        self.time_used += time_used
        
        if self.internet_plan:
            method_config = self.internet_plan.access_methods.get(self.access_method, {})
            
            data_limit = method_config.get('dataLimit', {})
            if data_limit.get('value', '').lower() != 'unlimited':
                self.remaining_data -= data_used
                if self.remaining_data < 0:
                    self.remaining_data = 0
                    self.status = 'suspended'
            
            usage_limit = method_config.get('usageLimit', {})
            if usage_limit.get('value', '').lower() != 'unlimited':
                self.remaining_time -= time_used
                if self.remaining_time < 0:
                    self.remaining_time = 0
                    self.status = 'suspended'
        
        self.last_activity = timezone.now()
        self.save()

    def get_remaining_data_display(self):
        if self.internet_plan:
            method_config = self.internet_plan.access_methods.get(self.access_method, {})
            data_limit = method_config.get('dataLimit', {})
            if data_limit.get('value', '').lower() == 'unlimited':
                return "Unlimited"
        
        bytes_val = self.remaining_data
        if bytes_val >= 1024 ** 3:
            return f"{(bytes_val / (1024 ** 3)):.2f} GB"
        elif bytes_val >= 1024 ** 2:
            return f"{(bytes_val / (1024 ** 2)):.2f} MB"
        elif bytes_val >= 1024:
            return f"{(bytes_val / 1024):.2f} KB"
        else:
            return f"{bytes_val} B"

    def get_remaining_time_display(self):
        if self.internet_plan:
            method_config = self.internet_plan.access_methods.get(self.access_method, {})
            usage_limit = method_config.get('usageLimit', {})
            if usage_limit.get('value', '').lower() == 'unlimited':
                return "Unlimited"
        
        seconds = self.remaining_time
        if seconds >= 3600:
            hours = seconds // 3600
            minutes = (seconds % 3600) // 60
            return f"{hours}h {minutes}m"
        elif seconds >= 60:
            minutes = seconds // 60
            return f"{minutes}m"
        else:
            return f"{seconds}s"

    def __str__(self):
        return f"Subscription for {self.client.user.username if self.client else 'Unknown'} - {self.internet_plan.name if self.internet_plan else 'No Plan'} ({self.access_method})"

    class Meta:
        verbose_name = 'Subscription'
        verbose_name_plural = 'Subscriptions'
        ordering = ['-start_date']
        indexes = [
            models.Index(fields=['client', 'start_date']),
            models.Index(fields=['internet_plan']),
            models.Index(fields=['is_active']),
            models.Index(fields=['status']),
            models.Index(fields=['mac_address']),
            models.Index(fields=['router']),
            models.Index(fields=['access_method']),
            models.Index(fields=['transaction_reference']),  
        ]
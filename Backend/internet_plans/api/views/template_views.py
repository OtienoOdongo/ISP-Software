...
class PlanTemplateListView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            # Serialize - FIXED: Use correct serializer
            serializer = PlanTemplateListSerializer(page, many=True, context={'request': request})
...
class CreatePlanFromTemplateView(APIView):
    ...
    def post(self, request, template_id):
        ...
        try:
            ...
            serializer = TemplateToPlanSerializer(
                data=request.data,
                context={'request': request, 'template_id': template_id}
            )
            ...
            return Response({
                ...
                'plan': {
                    ...
                    'plan_type': plan.plan_type,
                    'access_methods': plan.get_enabled_access_methods(),
                    'template_used': plan.template.name if plan.template else None,
                    'created_by': {
                        'id': str(request.user.id),
                        'email': request.user.email
                    }

...
Clean interface for service_operations app integration
Production-ready with comprehensive documentation
Updated: Added metadata, created_by; enhanced mock for production fallbacks; aligned with adapter impl.
"""
...
class ServiceOperationsInterface:
    ...
    This interface should be implemented by service_operations app
    to provide actual functionality.
    Updated: Added metadata to methods for session info; created_by for auditing.
    """
    ...
    def create_subscription(
        ...
        Args:
            ...
            router_id: Optional router ID for router-specific plans
            mac_address: Optional MAC address for hotspot
            metadata: Additional metadata (e.g., session info from captive portal)
            created_by: User/system creating the subscription for auditing
...
class MockServiceOperationsInterface(ServiceOperationsInterface):
    ...
    Mock implementation for development and testing
    Returns sample data without external dependencies
    Updated: Enhanced to match production params; added metadata handling; more realistic fallbacks.
    """
    ...
    def create_subscription(
        ...
        return {
            ...
            'end_date': (timezone.now() + timedelta(hours=duration_hours)).isoformat(),
            'transaction_reference': 'MOCK-TXN-001',
...
def get_service_operations_interface() -> ServiceOperationsInterface:
    ...
    try:
        ...
        logger.warning(f"Service operations app not available: {e}, using mock interface")
        return MockServiceOperationsInterface

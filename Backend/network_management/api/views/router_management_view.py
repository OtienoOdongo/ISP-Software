...
from internet_plans.models.create_plan_models import InternetPlan, Subscription
from payments.models.payment_config_model import Transaction
from payments.serializers.payment_config_serializer import MpesaCallbackConfigurationSerializer
from django.contrib.auth import get_user_model
...
class RouterActivateUserView(APIView):
    ...
    def post(self, request, pk):
        ...
        try:
            ...
            # Ensure a Subscription record exists (don't fail if missing)
            try:
    ...
    def calculate_remaining_time(self, client, plan):
        sessions = HotspotUser.objects.filter(client=client, plan=plan, active=False)
    ...
    def activate_on_router(self, router, hotspot_user):
        try:
            ...
            logger.exception(f"Error activating on router {getattr(router, 'id', 'unknown')}")
            return False, str(e)
...
class RestoreSessionsView(APIView):
    ...
    def post(self, request, pk):
        ...
        for session in interrupted_sessions:
            try:
                if getattr(session, "hotspot_user", None) and session.hotspot_user.remaining_time > 0:
                    hotspot_user = session.hotspot_user
                    hotspot_user.active = True
                    hotspot_user.router = router
                    hotspot_user.connected_at = timezone.now()
    ...
    def activate_on_router(self, router, hotspot_user):
        # reuse the same logic as other activate_on_router implementations
        try:
            if router.type == "mikrotik":
                api_pool = RouterOsApiPool(
                    router.ip,
...
class UserSessionRecoveryView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            if not recent_session:
                return Response({"error": "No session found for this device"}, status=status.HTTP_404_NOT_FOUND)

            if recent_session.remaining_time <= 0:
                return Response({"error": "No remaining time in your session"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            new_hotspot_user = HotspotUser.objects.create(
                ...
                active=True,
                remaining_time=recent_session.remaining_time,
                total_session_time=getattr(recent_session, "total_session_time", 0)
            )

            success, error = self.activate_on_router(router, new_hotspot_user)
            ...
            if success:
                ...
                new_hotspot_user.delete()
                return Response({"error": f"Failed to activate session: {error}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def activate_on_router(self, router, hotspot_user):
        # identical implementation as above; kept here to ensure view is self-contained
        try:
            if router.type == "mikrotik":
                api_pool = RouterOsApiPool(
                    router.ip,
...
# Add callback view per router
class RouterCallbackConfigView(APIView):
    permission_classes = [IsAuthenticated]

    def get(self, request, pk):
        router = get_object_or_404(Router, pk=pk)
        callbacks = router.mpesa_callbacks.all()  # Using related name on your model
        serializer = MpesaCallbackConfigurationSerializer(callbacks, many=True)
        return Response(serializer.data)

    def post(self, request, pk):
        data = request.data.copy()
        data['router'] = pk
        serializer = MpesaCallbackConfigurationSerializer(data=data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

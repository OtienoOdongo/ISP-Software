...
const HotspotPortal = () => {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      const [plansResponse, paymentMethodsResponse] = await Promise.all([
        // Get hotspot-enabled plans from internet_plans app
        api.get("/api/internet_plans/plans/public/?client_type=hotspot"),
        
        // Get available payment methods from payments app
        api.get("/api/payments/available-methods/")
      ]);
      ...
      // Handle plans response structure
      const plansData = plansResponse.data.plans || plansResponse.data.results || plansResponse.data || [];
      setPlans(plansData);
      
      // Handle payment methods response structure
      const methodsData = paymentMethodsResponse.data.available_methods || 
                         paymentMethodsResponse.data.results || 
  ...
  const getMacAddress = async () => {
    try {
      // Try to get MAC from URL parameters (common in hotspot systems)
      const urlParams = new URLSearchParams(window.location.search);
      const macFromUrl = urlParams.get("mac") || urlParams.get("mac_address") || urlParams.get("client_mac");
      if (macFromUrl) return macFromUrl;

      // Using network_management MAC detection endpoint
      const response = await api.get("/api/network_management/get-mac/");
      return response.data.mac_address || response.data.mac || "00:00:00:00:00:00";
    } catch (error) {
      console.error("Error fetching MAC address:", error);
      return "00:00:00:00:00:00";
    }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-teal-900 to-cyan-700 flex items-center justify-center">
        <div className="text-center">
          <div className="w-16 h-16 border-4 border-white border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
          <p className="text-white text-lg">Loading Hotspot Portal...</p>
          <p className="text-white text-sm mt-2">Connecting to server...</p>
          ...
          <div className="mt-8 p-6 bg-red-500/20 rounded-lg text-center max-w-2xl mx-auto">
            ...
            <div className="space-y-3">
              ...
              <div className="text-sm text-red-300">
                ...
                <ul className="list-disc list-inside text-left mt-2 space-y-1">
                  <li>Ensure Django server is running: <code className="bg-red-900 px-1 rounded">python manage.py runserver</code></li>
                  <li>Check if API is accessible: <a href="http://localhost:8000/api/internet_plans/public/?client_type=hotspot" target="_blank" rel="noopener noreferrer" className="underline">Test this link</a></li>
                  <li>Verify your API base URL in api/index.js</li>
                  <li>Check browser console for detailed errors</li>
                </ul>
              </div>

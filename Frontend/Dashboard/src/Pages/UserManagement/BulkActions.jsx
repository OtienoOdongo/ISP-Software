...
// BulkActions.jsx - COMPLETE VERSION
import React, { useState, useEffect, useMemo, useCallback } from 'react';
import { 
  Upload, Download, Send, Users, FileText, CheckCircle,
  AlertCircle, X, Filter, Search, UserCheck, UserX, Mail,
  MessageSquare, Clock, Edit3, Trash2, Plus, ChevronDown,
  ChevronUp, Phone, Loader, RefreshCw, Calendar, Zap
} from 'lucide-react';
import api from '../../api';
import { useAuth } from '../../context/AuthContext';
import { useTheme } from '../../context/ThemeContext';
...
const BulkActions = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [usersRes, templatesRes, historyRes, plansRes] = await Promise.all([
        api.get('/api/user_management/bulk-users/'),
        api.get('/api/user_management/message-templates/'),
        api.get('/api/user_management/bulk-history/'),
        api.get('/api/internet_plans/plans/') // Assuming this endpoint exists
      ]);
      ...
      setStats({
        ...
        last_import: usersRes.data.users?.length || 0
      });
      
      setSavedTemplates(templatesRes.data || []);
      setActionHistory(historyRes.data.actions || []);
      
      // Extract plan names from plans response
      const plans = plansRes.data.results || plansRes.data || [];
      setAvailablePlans(plans.map(plan => plan.name).filter(name => name));
                ...
                <div className={`p-4 rounded-lg border ${cardClass}`}>
                  ...
                  {importResults ? (
                    <div className="space-y-3">
                      <div className="flex flex-col sm:flex-row items-center gap-4">
                        {[
                          { label: 'Successful', value: importResults.success_count || 0, color: 'text-green-600 dark:text-green-500' },
                          { label: 'Failed', value: importResults.failed_count || 0, color: 'text-red-600 dark:text-red-500' },
                          { label: 'Total', value: importResults.total_users || 0, color: 'text-blue-600 dark:text-blue-500' }
                        ].map((stat, index) => (
                          <div key={index} className="text-center flex-1">
                            <div className={`text-2xl font-bold ${stat.color}`}>{stat.value}</div>
                            <div className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>{stat.label}</div>
                    ...
                    {actionHistory.length > 0 ? (
                      actionHistory.map((action, index) => (
                        <tr key={index} className={theme === 'dark' ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}>
                          ...
                          <td className="p-3 dark:text-gray-300 whitespace-nowrap">{formatDate(action.created_at)}</td>
                        </tr>
                      ))
                    ) : (
                      <tr>

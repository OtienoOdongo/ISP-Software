...
const TemplateTypeSelection = ({ 
  ...
  const templateTypes = [
    ...
    },
    {
      ...
      features: [
        { icon: Shield, text: "IP pool management" },
        { icon: Box, text: "Basic PPPoE configuration" },
        { icon: TrendingDown, text: "Technical specifications only" },
        { icon: Users, text: "Stable connection management" }
      ]
    }
  ];
  ...
  return (
    <div className={`p-4 sm:p-6 rounded-xl shadow-lg border ${themeClasses.bg.card} ${themeClasses.border.light}`}>
      <h2 className="text-lg lg:text-xl font-semibold mb-3 sm:mb-4 flex items-center">
        <Box className="w-4 h-4 sm:w-5 sm:h-5 mr-2 sm:mr-3 text-indigo-600 flex-shrink-0" />
        Select Template Type
      </h2>
      
      <p className={`text-xs sm:text-sm mb-4 sm:mb-6 ${themeClasses.text.secondary}`}>
        Choose the type of template you want to create. Both options provide comprehensive configuration for their respective access methods.
      </p>
        ...
        {templateTypes.map((type) => {
          ...
          return (
            <motion.button
              ...
              onClick={() => onTemplateTypeSelect(type.id)}
              onKeyDown={(e) => handleKeyPress(e, type.id)}
              className={`w-full p-4 sm:p-6 border-2 rounded-xl text-left transition-all duration-200 focus:outline-none focus:ring-4 focus:ring-indigo-500/50 ${
                colorClasses.border
              } ${colorClasses.bg}`}
            >
              <div className="flex items-start mb-3 sm:mb-4">
                <div className={`p-2 sm:p-3 rounded-lg flex-shrink-0 ${colorClasses.iconBg}`}>
                  <IconComponent className={`w-4 h-4 sm:w-6 sm:h-6 ${colorClasses.icon}`} />
                </div>
                <div className="ml-3 sm:ml-4 flex-1 min-w-0">
                  <h3 className={`text-base sm:text-lg font-semibold mb-1 ${colorClasses.text.primary} truncate`}>
                    {type.name}
                  </h3>
                ...
                {type.features.map((feature, index) => {
                  ...
                  return (
                    <div key={index} className="flex items-center text-xs sm:text-sm">
                      <FeatureIcon className={`w-3 h-3 sm:w-4 sm:h-4 mr-2 ${colorClasses.text.secondary} flex-shrink-0`} />
              ...
              {isSelected && (
                <div className="flex justify-end mt-3 sm:mt-4">
                  <div 
                    className={`p-1 sm:p-2 rounded-full ${colorClasses.badge}`}
                    aria-hidden="true"
                  >
      ...
      {!templateType && (
        <motion.div 
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
          className="mb-3 sm:mb-4 p-2 sm:p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg"
        >
      ...
      <div className="flex justify-end">
        <motion.button
          ...
          onClick={onContinue}
          disabled={!templateType}
          className={`px-4 sm:px-6 py-2 sm:py-3 rounded-lg flex items-center font-medium transition-all duration-200 focus:outline-none focus:ring-4 focus:ring-indigo-500/50 text-sm sm:text-base ${
            !templateType 
              ? 'bg-gray-400 text-gray-700 cursor-not-allowed dark:bg-gray-600 dark:text-gray-300' 
              : `${themeClasses.button.primary} hover:shadow-lg`
          }`}
          ...
          aria-disabled={!templateType}
        >
          <span className="truncate">Continue to {getSelectedTypeName()} Configuration</span>

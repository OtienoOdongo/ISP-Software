...
//       // Increment usage count in background with error handling
//       try {
...
//                 usage_count: (t.usage_count || t.usageCount || 0) + 1
//               }
//             : t
//         ));
//       } catch (usageError) {
//         console.warn("Failed to increment template usage:", usageError);
...
//   // Enhanced template creation with duplication prevention and better error handling
//   const handleCreateFromTemplate = async (template) => {
//     try {
//       const planName = prompt("Enter plan name:", `${template.name} - ${new Date().toLocaleDateString()}`);
...
//       // Update template usage count locally immediately
//       setTemplates(prev => prev.map(t => 
//         t.id === template.id 
//           ? { 
//               ...t, 
...
//       onCreateFromTemplate(response.data);
//       showToast(`Plan "${planName}" created successfully!`);
...
//   const QuickCreateButton = ({ template, onQuickCreate }) => (
//     <motion.button
...
//       title="Quick Create Plan from Template"
//     >
...
const PlanTemplates = ({ templates: initialTemplates, onApplyTemplate, onCreateFromTemplate, onBack, theme }) => {
  ...
  // Enhanced template creation with better usage tracking
  const handleCreateFromTemplate = async (template, planName = null) => {
    try {
      ...
      // Enhanced usage count update - update both fields
      setTemplates(prev => prev.map(t => 
        t.id === template.id 
          ? { 
              ...t, 

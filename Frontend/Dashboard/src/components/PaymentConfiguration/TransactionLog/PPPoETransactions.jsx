...
// import { FaNetworkWired, FaSearch, FaDownload } from 'react-icons/fa';
...
//   const [pppoeStats, setPppoeStats] = useState({
//     total: 0,
//     success: 0,
//     pending: 0,
//     failed: 0,
...
//   // Fetch PPPoE-specific transactions
//   const fetchPppoeTransactions = useCallback(async () => {
//     const params = {
//       start_date: filters.startDate.toISOString().split('T')[0],
//       end_date: filters.endDate.toISOString().split('T')[0],
//       status: filters.status !== 'all' ? filters.status : undefined,
//       payment_method: filters.paymentMethod !== 'all' ? filters.paymentMethod : undefined,
//       access_type: 'pppoe', // Always filter by PPPoE
...
//       setPppoeTransactions(transactionsData);
...
//       // Calculate PPPoE-specific stats
//       const stats = response.data.stats || {};
//       const pppoeStatsData = stats.byAccessType?.pppoe || {};
      
//       setPppoeStats({
//         total: pppoeStatsData.count || transactionsData.length,
//         success: pppoeStatsData.success_count || transactionsData.filter(t => t.status === 'success').length,
//         pending: pppoeStatsData.pending_count || transactionsData.filter(t => t.status === 'pending').length,
//         failed: pppoeStatsData.failed_count || transactionsData.filter(t => t.status === 'failed').length,
//         totalAmount: pppoeStatsData.total_amount || transactionsData.reduce((sum, t) => sum + (parseFloat(t.amount) || 0), 0)
//       });

//     } catch (err) {
//       console.error('Failed to fetch PPPoE transactions:', err);
//     }
//   }, [filters]);

//   useEffect(() => {
//     fetchPppoeTransactions();
//   }, [fetchPppoeTransactions]);
...
//   const generatePppoeReport = async () => {
//     try {
//       const params = {
//         start_date: filters.startDate.toISOString().split('T')[0],
//         end_date: filters.endDate.toISOString().split('T')[0],
...
//       if (data.length === 0) {
//         alert('No PPPoE transactions available for report generation');
//         return;
//       }

//       const doc = new jsPDF();
//       doc.setFontSize(16);
//       doc.text('PPPOE TRANSACTIONS REPORT', 105, 15, { align: 'center' });
...
//       doc.save(`PPPoE_Transactions_${new Date().toISOString().slice(0, 10)}.pdf`);
//     } catch (err) {
//       console.error('PPPoE PDF generation error:', err);
//       alert('Failed to generate PPPoE report');
//     }
//   };

//   const fetchPppoeExportData = async () => {
//     try {
//       const params = {
//         start_date: filters.startDate.toISOString().split('T')[0],
//         end_date: filters.endDate.toISOString().split('T')[0],
...
//     } catch (err) {
//       console.error('PPPoE export data error:', err);
//       return [];
//     }
//   };

//   const pppoeStatsCards = [
//     {
//       label: 'Total PPPoE Transactions',
//       value: pppoeStats.total,
//       icon: FaNetworkWired,
//       color: 'green',
...
//     {
//       label: 'Total Revenue',
//       value: formatCurrency(pppoeStats.totalAmount),
//       icon: FaDownload,
//       color: 'indigo',
...
//     {
//       label: 'Success Rate',
//       value: pppoeStats.total > 0 ? `${((pppoeStats.success / pppoeStats.total) * 100).toFixed(1)}%` : '0%',
//       icon: FiTrendingUp,
//       color: 'purple',
...
//           <div className={`p-3 rounded-lg ${theme === 'dark' ? 'bg-green-900/30 text-green-400' : 'bg-green-100 text-green-600'}`}>
...
//               Wired network access transactions and analytics
//             </p>
//           </div>
//         </div>
...
//           {pppoeStatsCards.map((stat, index) => {
...
//             <input
//               type="text"
//               placeholder="Search PPPoE transactions..."
...
//             <CSVLink
//               data={async () => {
//                 const data = await fetchPppoeExportData();
//                 return data.map(tx => ({
...
//                 }));
//               }}
//               filename={`pppoe_transactions_${new Date().toISOString().slice(0, 10)}.csv`}
//               className="px-4 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors flex items-center gap-2 text-center"
//             >
//               <FaDownload />
//               Export CSV
//             </CSVLink>
...
//               {pppoeTransactions.length === 0 ? (
//                 <tr>
//                   <td colSpan="8" className="px-6 py-8 text-center">
//                     <div className="flex flex-col items-center justify-center">
//                       <FaNetworkWired className="text-4xl text-gray-400 mb-2" />
//                       <p className={theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}>
//                         No PPPoE transactions found
//                       </p>
//                     </div>
//                   </td>
//                 </tr>
//               ) : (
//                 pppoeTransactions.map((tx, index) => (
...
//               {pagination.total_items} PPPoE transactions
//             </div>
...
// export default PPPoETransactions;

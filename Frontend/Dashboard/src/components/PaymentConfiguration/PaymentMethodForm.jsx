...
const PaymentMethodForm = ({
  ...
  const handleToggleSecret = (index) => (fieldName) => {
    if (typeof onToggleSecret === 'function' && fieldName && index !== undefined) {
      onToggleSecret(index, fieldName);
    }
  };

  const handleToggleAdvanced = (index) => () => {
    if (typeof onToggleAdvanced === 'function' && index !== undefined) {
      onToggleAdvanced(index);
    }
  };

  const handleGenerateCallback = (index) => () => {
    if (typeof onGenerateCallback === 'function' && index !== undefined) {
      onGenerateCallback(index);
    }
  };
        ...
        <div key={index} className={activeTab === index ? 'block animate-fadeIn' : 'hidden'}>
          ...
          <div className="px-6 pb-6">
            <MethodFields
              ...
              onChange={onChange}
              onToggleSecret={handleToggleSecret(index)} 
              onToggleAdvanced={handleToggleAdvanced(index)}
              onGenerateCallback={handleGenerateCallback(index)}
              onCopyToClipboard={onCopyToClipboard}
              theme={theme}
            />
          </div>
